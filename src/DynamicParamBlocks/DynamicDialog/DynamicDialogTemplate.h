...
#include <atlbase.h>				// This is part of the ATL library, only available with VS Standard Ed.
#include "WTL8.0\include\atlapp.h"	// These are part of the WTL libraries, included with this project.
#include "WTL8.0\include\atldlgs.h"	// These are part of the WTL libraries, included with this project.
...
public:
	...
	operator DLGTEMPLATEEX*()	{ return this->GetDLGTEMPLATE(); }
	...
	DLGTEMPLATEEX* GetDLGTEMPLATE(void)
	{
		DLGTEMPLATEEX* dialogTemplate = (DLGTEMPLATEEX*)::GlobalLock(m_hDialogTemplateMemory);
...
public:
			...
			UINT fontSize = 8)
	{
		...
		if(m_hDialogTemplateMemory)
		{
			...
			DLGTEMPLATEEX* pDialogTemplate = (DLGTEMPLATEEX*) ::GlobalLock(m_hDialogTemplateMemory);
			if(pDialogTemplate)
			{
				// Define dialog template
				pDialogTemplate->dlgVer = 1;
				pDialogTemplate->signature = 0xFFFF;
				pDialogTemplate->style = style;
				pDialogTemplate->exStyle = styleEx;
				
				pDialogTemplate->cDlgItems = 0;
				pDialogTemplate->x = 0;
				pDialogTemplate->y = 0;
			...
			const wchar_t* classStringOrAtom, size_t classStringOrAtomWordCount)
	{
		...
		if(pDialogTemplate)
		{
			...
			DLGITEMTEMPLATEEX* pDialogItem = (DLGITEMTEMPLATEEX*) (((DWORD_PTR)pOffset + 3) & ~3);
			pDialogItem->style = style;
			pDialogItem->exStyle = dwExtendedStyle;
			pDialogItem->x  = x;
			pDialogItem->y  = y;
			pDialogItem->cx = cx;
			pDialogItem->cy = cy;
...
public:
	...
	INT_PTR DoModal(HWND hWndParent = ::GetActiveWindow(), LPARAM dwInitParam = NULL)
	{
		...
		_ASSERTE((bool)m_dynamicDialogTemplate);
	...
	BOOL EndDialog(int nRetCode)
	{
		...
		_ASSERTE(m_bModal);	// must be a modal dialog
	...
	{
		_ASSERTE(::IsWindow(m_hWnd));
		_ASSERTE(!m_bModal);	// must not be a modal dialog
